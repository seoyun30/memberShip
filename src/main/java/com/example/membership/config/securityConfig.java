        ...
        // 페이지 접속 권한
                .authorizeHttpRequests(
                        authorization -> authorization
                                .requestMatchers("/user/login/**").permitAll()  //로그인페이지는 누구나 접속가능한 권한
                                .requestMatchers("/board/register").authenticated()  //로그인 한 사람만 접속 가능
                                .requestMatchers("/item/register").hasRole("ADMIN")
                                .requestMatchers("/user/list").hasRole("ADMIN")
                                .anyRequest().permitAll()  //그 외 다 열어
//                                .anyRequest().authenticated() //그 외에는 다 로그인해서 접속해
        ...
        //위변조 방지 웹에서 from태그 변경 등의 변조를 방지
                ...
                .logout(
                        logout->logout
                                ...
                                .logoutSuccessUrl("/")                  //localhost:8090으로 간다.
                                                                            //dns 주소일 경우 www.naver.com까지로 감
                                                                            //컨트롤러에서 만들어줄 예정?
                )

        //예외처리 // 로그인 되지 않은 사용자, 권한이 없는 사용자 접속시 취할 행동들
//                .exceptionHandling(
//                        a->a.
//                )
